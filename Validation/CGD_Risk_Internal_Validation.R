...
for(i in 1:cv_repeats){
  ...
  for(j in 1:numfolds){
    ...
    cgd_model <- catboost.train(learn_pool = train_fold, params = params)
    
    tfold_preds <- catboost.predict(model = cgd_model, pool = train_fold, prediction_type = 'Probability')
    vfold_preds <- catboost.predict(model = cgd_model, pool = valid_fold, prediction_type = 'Probability')
...
cgd_calibration <- ggarrange(calib_plt, predprob_plt, nrow = 2, ncol = 1, heights = c(1, 0.30))
cgd_calibration
...
PI_loss_plt <- PI_df %>%
               ...
               labs(title = "CGD-Risk Predictor Importance", x = "Predictor", y = "Difference in Loss")
