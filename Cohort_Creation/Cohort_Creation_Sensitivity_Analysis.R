# Cohort Filtering for the CHARLS sensitivity analysis
...
# Filter by age >= 50 for elderly, keep NA values
df <- df %>% filter(age >= age_cutoff_low | is.na(age))
df <- df %>% filter(age <= age_cutoff_high | is.na(age))
...
# Saving the data set
write.csv(df, "Data/CHARLS_sensitivity.csv", row.names = FALSE)